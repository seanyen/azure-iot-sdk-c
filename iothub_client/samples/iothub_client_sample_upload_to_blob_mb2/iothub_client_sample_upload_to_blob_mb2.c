...
int main(void)
{
    ...
    device_handle = IoTHubDeviceClient_CreateFromConnectionString(connectionString, HTTP_Protocol);
    ...
    else
    {
        ...
        if (proxyHost != NULL && IoTHubDeviceClient_SetOption(device_handle, OPTION_HTTP_PROXY, &http_proxy_options) != IOTHUB_CLIENT_OK)
        {
            (void)printf("failure to set proxy\n");
        }
        else
        {
            if (IoTHubDeviceClient_UploadMultipleBlocksToBlobAsync(device_handle, "subdir/hello_world_mb.txt", getDataCallback, NULL) != IOTHUB_CLIENT_OK)
            {
